...
int main(){
    ...
    std::vector<char> vec = Utils::serialize<std::string>(msg);
    ...
    std::vector<char> ser_es = Utils::serialize<Utils::ex_struct>(es);
    Utils::printVector(ser_es);

    Utils::ex_struct es_des = Utils::deserialize<Utils::ex_struct>(ser_es);
    std::cout << ntohs(es_des.a) << std::endl;

    auto zwyciestwo = Utils::deserialize<std::string>(vec1);
